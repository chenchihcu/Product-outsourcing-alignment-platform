...
export default function PrintReport({ data }) {
  ...
  return (
    <div className="print-report-container">
      ...
      <table className="print-table">
        <thead>
          <tr>
            ...
            <th style={{ width: '45%' }}>規格描述 / 加工廠確認</th>
            <th style={{ width: '25%' }}>提供數量</th>
          </tr>
        </thead>
        <tbody>
          {bi.processItems?.smt && (
      ...
      {pc.keyParts?.has && (
        <div className="print-subsection-block">
          ...
          {deliverySections.map((section) => (
            <Fragment key={section.label}>
              ...
              {section.records.map((rec) => (
                <tr key={rec.id}>
                  <td className="delivery-item">{section.formatName ? section.formatName(rec) : rec.name}</td>
      ...
      <table className="print-table print-pending-table">
        <thead>
          <tr>
            <th style={{ width: '28%' }}>範圍</th>
            <th style={{ width: '47%' }}>項目</th>
          ...
          {pendingItems.length > 0 ? (
            pendingItems.map((item, idx) => (
              <tr key={`${item.scope}-${item.item}-${idx}`}>
                <td>{item.scope}</td>
                <td>{item.item}</td>
                <td><span className="badge badge-danger">{item.status}</span></td>
              </tr>
            ))
          ) : (
            <tr>
